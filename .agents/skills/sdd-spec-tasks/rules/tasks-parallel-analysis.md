...
**Policy: `(P)` is an execution contract**, not an informational note. Marking `(P)` promises that `sdd-impl` **may** parallel-dispatch this task's Wave/batch with other ready `(P)` peers when:
