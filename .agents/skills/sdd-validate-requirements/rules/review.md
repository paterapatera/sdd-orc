...
- Derive the questions in `.agents/skills/sdd-spec-requirements/rules/quality.md` from this feature's elements yourself, and apply its 「When a criterion does not settle the question」. Each question, each `## Checks` line, and each new screen is settled by a criterion whose result fails the bad implementation named in that file, or by a sourced `out:`. A screen name without the place that opens it, its items, destinations, and failure place is not settled. Another screen's destination does not name that opening place. A start place read off a different line, a removal settled only by 「保存されない」 on another failure, or a limit whose number is not in the criterion text, is not settled. A cited criterion that would not fail it, an `out:` whose source does not say it, or a missing line is NO-GO. Write it in Findings as a rollback to grill. Do not park it under Accepted residual risks.
