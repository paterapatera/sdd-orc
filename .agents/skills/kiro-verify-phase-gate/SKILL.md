---
name: kiro-verify-phase-gate
description: Verify spec phase gate readiness (要求/設計/タスク) with fresh artifact and VERDICT evidence. Use at AI-DLC pre-implementation phase gates instead of FEATURE_GO. Implementation completion uses /kiro-verify-completion with FEATURE_GO.
metadata:
  shared-rules: "../kiro-validate-shared/phase-gate.md"
---

# kiro-verify-phase-gate

<background_information>
Pre-implementation phase gates (requirements, design, tasks) must **not** use `FEATURE_GO` from `/kiro-verify-completion` — that claim type requires tests, runtime smoke, and integration evidence.
...
Do not open the human approval gate. Re-run the failing validate or generation step per `kiro-orchestrate/rules/rollback.md` § Phase gate failures.
</instructions>
