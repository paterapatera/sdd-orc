...
- **Batch / selection completion** (primary inside `/sdd-impl`): once immediately before marking all tasks in a batch (or all selected manual tasks) `[x]` — claim type `BATCH` (or multi-`TASK`)
- **Single manual task**: once at that task's end before `[x]` — claim type `TASK`
- **Feature end**: after `/sdd-validate-impl` returns GO, before reporting feature success — claim type `FEATURE_GO`
