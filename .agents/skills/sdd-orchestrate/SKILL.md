...
2. After routing, compute complexity tier per `rules/complexity-tier.md` (on 要求新規作成: **after** the brief grill `READY`, per `flows.md` § 要求新規作成 entry — this applies to S candidates too; grill answers may turn S into M), map tier → path (`flows.md` § Orchestration Paths by Tier), and select S/M/L flow variant (write `complexity_tier` / `complexity_score` / `complexity_rationale` to `spec.json`).
...
2. `/sdd-grill <feature> brief` (runs before the tier is known; `WAITING` → Grill 待ち stop)
3. Tier computed as S from the grilled brief
4. Dispatch `/sdd-spec-quick <feature>`
5. `QUICK: DONE` → **Terminal auto-approve (S)** per `rules/gates.md` → PR Summary Output → end. `QUICK: ESCALATE_M` → set tier M and continue the 要求ブロック (`flows.md` § 要求新規作成 (S)). `QUICK: FOLLOW_UP` → stop and report

Do **NOT** dispatch individual `spec-requirements`, `validate-*`, `spec-design`, `spec-tasks` separately on S tier.

**For M / L (and non-S flows):**

1. **Dispatch** the listed `/sdd-*` skill — do not inline the role's work. 要求 runs as the **要求ブロック** (`rules/flows.md` § 要求ブロック): `/sdd-grill brief` → `/sdd-spec-requirements` → `/sdd-grill req` → `/sdd-validate-requirements`, entered at the step the artifact-state table picks.
2. **Parse outcome** from report files (`VERDICT:`), the grill's `GRILL: READY | WAITING | BLOCKED` line + grill file header, or a generation skill's result line (`DESIGN:` / `TASKS:`; a `RETURN_TO_*` or `STOPPED` line → `rules/rollback.md` § Generation stops). Report paths: `../sdd-validate-shared/contract.md` (read only when parsing). Require unified `*-review.md` (`VERDICT:` + Phase Gate `STATUS:`); old 4-file-only specs are not GO.
3. **On phase gates all pass** (要求: the 要求ブロック entry table reaches Phase terminal — grills READY and not stale, `requirements-review.md` GO + Phase Gate `VERIFIED`; 設計: `design-review.md` Phase Gate `VERIFIED`) → **Phase terminal** per `rules/gates.md`（handoff → stop; do **not** dispatch the next phase）. If the user then sends correction notes in this chat, stay in the phase. A new `/sdd-orchestrate <feature>` is how they proceed.
4. **On タスク** after generation → **[調整者] タスクゲート** (`rules/gates.md`; three checks read fresh from disk) → on `VERIFIED`, **Terminal auto-approve** (set `ready_for_implementation: true`) → PR Summary → end.
...
- **S tier (quick-path):** brief grill (as for every 要求新規作成), then one dispatch to `/sdd-spec-quick`, then **Terminal auto-approve (S)** → PR Summary → end. Do not run L-flow steps individually.
...
- Spec state for the Modification / Upstream Dependency Guards: `/sdd-spec-status <feature>` (internal; returns a status block, not a user report).
...
| Role | Skills |
| ---- | ------ |
| プロダクトオーナー | spec-requirements (includes init Step 0), validate-requirements (unified); S tier: via `spec-quick` |
| セキュリティ管理者 | (requirements/design sec via unified validate-requirements / validate-design-qa) |
| 設計者 | spec-design (includes brownfield gap), validate-design-qa (unified), spec-tasks; S tier: via `spec-quick` |
| 品質管理者 | validate-requirements (unified), validate-design-qa (unified design) |
| 業務委託担当者 | grill `brief` (every 要求新規作成 incl. S, and 要求更新), grill `req` (M/L 要求ブロック only) |
| アーキテクト管理者 | (arch via unified validate-design-qa) |
| 調整者 (self) | routing, gates (タスクゲート inline; 要求/設計は統合 validate 内), rollback |

`discovery` is **not dispatched** by the orchestrator — `/sdd-discovery` is an external pre-step run standalone before orchestration (`routing.md` § Entry Contract). `/sdd-grill <feature> brief` is dispatched at 要求新規作成 entry for every tier (S included) and in the 要求更新 要求ブロック. `/sdd-grill <feature> req` is dispatched only in the M/L 要求ブロック (and after `QUICK: ESCALATE_M`).
</instructions>
