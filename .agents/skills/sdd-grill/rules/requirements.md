...
Options are results a user sees, not status codes or mechanisms. When the human answers that a characteristic has no condition, write `out: <their words> (source: grill:<id>)` on that line.
