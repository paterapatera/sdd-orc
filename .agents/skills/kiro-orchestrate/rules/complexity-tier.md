...
4. Write `complexity_tier` / `complexity_score` / `complexity_rationale` to `docs/specs/<feature>/spec.json` (create `spec.json` only if init already ran or will run via `/kiro-spec-requirements` Step 0; if pre-init, write immediately after Step 0 creates it, or stash the computed values and write on first `[調整者]` touch of `spec.json`)
...
When `/kiro-spec-design` classifies scope scale, prefer `complexity_tier` over brief heuristics:
