...
- [ ] {{NUMBER}}. {{TASK_DESCRIPTION}}{{PARALLEL_MARK}}
  ...
  - _Wave: {{N}}_ *(Required when this major task is itself executable. Phase order only; `sdd-impl` dispatches packed batches of consecutive majors.)*
...
- [ ] {{MAJOR_NUMBER}}.{{SUB_NUMBER}} {{SUB_TASK_DESCRIPTION}}
  ...
  - _Wave: {{N}}_ *(Required on every executable sub-task. Phase order only — `sdd-impl` dispatches packed batches of consecutive majors, not by Wave.)*
...
>
> **Wave annotation**: Assign `_Wave: N_` so Foundation → Core → Integration → Validation increase by dependency. Keep Integration/Validation in their own majors. `(P)` work with different `_Boundary:_` must be different majors (enables cross-major parallel dispatch). Wave is not the dispatch unit; `sdd-impl` packs consecutive skinny serial majors into one implementer.
>
