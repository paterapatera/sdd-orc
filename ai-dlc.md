...
| -------- | -------- | ---------------- | ---------------- |
| 要求 | `requirements.md` 生成済み + 要求 validate 2種すべて GO + 人間承認 | `approvals.requirements.generated: true` → 承認後 `approved: true` | `/kiro-spec-design` |
...
| `/kiro-validate-requirements-sec` | `/kiro-spec-requirements` または `requirements.md` | 修正後 `validate-requirements` → `sec` の順で再 validate |
...
### validate スキル（5種 specialist + 1 final gate・実装済み）
...
**実行順（直列・順序固定）**: `validate-requirements` → `sec`
...
7. **[ゲート] 要求フェーズ**: 調整者が validate 2種の GO・成果物・**決定事項サマリー**（各レポートの `## Decisions` 要約）を報告し、ユーザー承認を待つ。承認後 `approvals.requirements.approved: true`
8. [設計者]: `/kiro-validate-gap` を実行する（brownfield のみ・任意）
9. [設計者]: `/kiro-spec-design` を実行する
10. [品質管理者]: `/kiro-validate-design-qa` を実行する
11. [アーキテクト管理者]: `/kiro-validate-design-arch` を実行する（qa 通過・`design.md` 反映後）
12. [セキュリティ管理者]: `/kiro-validate-design-sec` を実行する（arch 通過・`design.md` 反映後）
13. [設計者]: `/kiro-validate-design-ex` を実行する。qa / arch / sec レポートを入力に総合 GO/NO-GO を判定。**最終ゲート**
14. [調整者]: `/kiro-verify-phase-gate <feature> design` を実行する
15. **[ゲート] 設計フェーズ**: **決定事項サマリー**を含めてユーザー承認を待つ。承認後 `approvals.design.approved: true`
16. [設計者]: `/kiro-spec-tasks` を実行する
17. [調整者]: `/kiro-verify-phase-gate <feature> tasks` を実行する
18. **[ゲート] タスクフェーズ**: **決定事項サマリー**（`tasks.md` と spec-tasks サマリーから合成）を含めてユーザー承認を待つ。承認後 `approvals.tasks.approved: true`
19. [実装者]: `/kiro-impl` を実行する（タスクごとに `/kiro-review` → `/kiro-verify-completion`）
20. [品質管理者]: `/kiro-validate-impl` を実行する
21. [調整者]: `/kiro-verify-completion`（`FEATURE_GO`）を実行する
22. **[ゲート] 実装フェーズ**: **決定事項サマリー**を含めてユーザー承認を待つ
23. 実装完了後、必要に応じて `/kiro-docs <feature>` でドキュメント集約・spec 整理を行う（任意・別実行）
24. 終了
...
5. [調整者]: `/kiro-verify-phase-gate <feature> requirements` を実行する
6. **[ゲート] 要求フェーズ**: **決定事項サマリー**を含めてユーザー承認を待つ
7. [設計者]: `/kiro-spec-design` を実行する（要求差分のみ更新）
8. [品質管理者]: `/kiro-validate-design-qa` を実行する（更新部分）
9. [アーキテクト管理者]: `/kiro-validate-design-arch` を実行する（更新部分・qa 反映後）
10. [セキュリティ管理者]: `/kiro-validate-design-sec` を実行する（更新部分・arch 反映後）
11. [設計者]: `/kiro-validate-design-ex` を実行する。**最終ゲート**
12. [調整者]: `/kiro-verify-phase-gate <feature> design` を実行する
13. **[ゲート] 設計フェーズ**: **決定事項サマリー**を含めてユーザー承認を待つ
14. [設計者]: `/kiro-spec-tasks` を実行する（更新部分のみタスク追加・上書き）
15. [調整者]: `/kiro-verify-phase-gate <feature> tasks` を実行する
16. **[ゲート] タスクフェーズ**: **決定事項サマリー**を含めてユーザー承認を待つ
17. [実装者]: `/kiro-impl` を実行する
18. [品質管理者]: `/kiro-validate-impl` を実行する
19. [調整者]: `/kiro-verify-completion`（`FEATURE_GO`）を実行する
20. **[ゲート] 実装フェーズ**: **決定事項サマリー**を含めてユーザー承認を待つ
21. 終了
